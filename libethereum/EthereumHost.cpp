...
public:
    ...
    void onPeerNewHashes(NodeID const& _peerID, std::vector<std::pair<h256, u256>> const& _hashes) override
    {
        try
        {
...
EthereumHost::EthereumHost(shared_ptr<p2p::CapabilityHostFace> _host, BlockChain const& _ch, OverlayDB const& _db,
...
void EthereumHost::maintainTransactions()
{
    ...
    auto ts = m_tq.topTransactions(c_maxSendTransactions);
    {
        ...
        for (size_t i = 0; i < ts.size(); ++i)
        {
            ...
            auto peers = get<1>(randomSelection(0, [&](EthereumPeerStatus const& status) { return status.m_requireTransactions || (unsent && !status.m_knownTransactions.count(t.sha3())); }));
    ...
    for (auto& peer : m_peers)
    {
        ...
        {
            RLPStream ts;
            m_host->prep(peer.first, CapDesc{name(), version()}, ts, TransactionsPacket, n).appendRaw(b, n);
...
*/
tuple<vector<NodeID>, vector<NodeID>> EthereumHost::randomSelection(unsigned _percent, std::function<bool(EthereumPeerStatus const&)> const& _allow)
{
    ...
    if (detailsFrom.totalDifficulty < detailsTo.totalDifficulty)
    {
        if (diff(detailsFrom.number, detailsTo.number) < 20)
        {
            ...
            for (NodeID const& peerID: get<0>(s))
                ...
                {
                    RLPStream ts;
                    m_host->prep(peerID, CapDesc{name(), version()}, ts, NewBlockPacket, 2).appendRaw(m_chain.block(b), 1).append(m_chain.details(b).totalDifficulty);
            ...
            {
                RLPStream ts;
                m_host->prep(peerID, CapDesc{name(), version()}, ts, NewBlockHashesPacket, blocks.size());
                for (auto const& b: blocks)
                {
                    ts.appendList(2);
                    ts.append(b);
...
void EthereumHost::onConnect(p2p::NodeID const& _peerID, u256 const& _peerCapabilityVersion)
{
    ...
    requestStatus(_peerID, m_networkId, m_chain.details().totalDifficulty, m_chain.currentHash(), m_chain.genesisHash());
}
...
bool EthereumHost::interpretCapabilityPacket(NodeID const& _peerID, unsigned _id, RLP const& _r)
{
    ...
    try
    {
        ...
        case StatusPacket:
        {
            ...
            LOG(m_logger) << "Status: " << peerStatus.m_protocolVersion << " / " << peerStatus.m_networkId << " / "
                << peerStatus.m_genesisHash << ", TD: " << peerStatus.m_totalDifficulty << " = " << peerStatus.m_latestHash;
            setIdle(_peerID);
        ...
        case GetBlockHeadersPacket:
        {
            ...
            RLPStream s;
            m_host->prep(_peerID, CapDesc{name(), version()}, s, BlockHeadersPacket, rlpAndItemCount.second).appendRaw(rlpAndItemCount.first, rlpAndItemCount.second);
        ...
        case GetBlockBodiesPacket:
        {
            ...
            m_host->addRating(_peerID, 0);
            RLPStream s;
            m_host->prep(_peerID, CapDesc{name(), version()}, s, BlockBodiesPacket, rlpAndItemCount.second).appendRaw(rlpAndItemCount.first, rlpAndItemCount.second);
            m_host->sealAndSend(_peerID, s);
            break;
        }
        ...
        case NewBlockHashesPacket:
        {
            ...
            {
                m_host->disableCapability(_peerID, CapDesc{name(), version()}, "Too many new hashes");
                break;
            }
        ...
        case GetReceiptsPacket:
        {
            ...
            m_host->addRating(_peerID, 0);
            RLPStream s;
            m_host->prep(_peerID, CapDesc{name(), version()}, s, ReceiptsPacket, rlpAndItemCount.second).appendRaw(rlpAndItemCount.first, rlpAndItemCount.second);
            m_host->sealAndSend(_peerID, s);
            break;
        }
        case NodeDataPacket:
...
void EthereumHost::setAsking(NodeID const& _peerID, Asking _a)
{
    ...
    m_host->addNote(_peerID, "sync", string(isCriticalSyncing(_peerID) ? "ONGOING" : "holding") + (needsSyncing(_peerID) ? " & needed" : ""));
}
...
bool EthereumHost::isCriticalSyncing(p2p::NodeID const& _peerID) const
{
    ...
    return peerStatus.m_asking == Asking::BlockHeaders || peerStatus.m_asking == Asking::State || (peerStatus.m_asking == Asking::BlockBodies && peerStatus.m_protocolVersion == 62);
}

void EthereumHost::requestStatus(p2p::NodeID const& _peerID, u256 _hostNetworkId, u256 _chainTotalDifficulty, h256 _chainCurrentHash, h256 _chainGenesisHash)
{
    auto itPeerStatus = m_peers.find(_peerID);
    if (itPeerStatus == m_peers.end())
        return;
    ...
    RLPStream s;
    m_host->prep(_peerID, CapDesc{name(), version()}, s, StatusPacket, 5) << protocolVersion() << _hostNetworkId << _chainTotalDifficulty
        << _chainCurrentHash << _chainGenesisHash;
    m_host->sealAndSend(_peerID, s);
}

void EthereumHost::requestBlockHeaders(p2p::NodeID const& _peerID, unsigned _startNumber, unsigned _count, unsigned _skip, bool _reverse)
{
    auto itPeerStatus = m_peers.find(_peerID);
    if (itPeerStatus == m_peers.end())
        return;
    ...
    RLPStream s;
    m_host->prep(_peerID, CapDesc{name(), version()}, s, GetBlockHeadersPacket, 4) << _startNumber << _count << _skip << (_reverse ? 1 : 0);
...
void EthereumHost::requestBlockHeaders(p2p::NodeID const& _peerID, h256 const& _startHash, unsigned _count, unsigned _skip, bool _reverse)
{
    auto itPeerStatus = m_peers.find(_peerID);
    if (itPeerStatus == m_peers.end())
        return;
    ...
    setAsking(_peerID, Asking::BlockHeaders);
    RLPStream s;
    m_host->prep(_peerID, CapDesc{name(), version()}, s, GetBlockHeadersPacket, 4) << _startHash << _count << _skip << (_reverse ? 1 : 0);
...
void EthereumHost::requestByHashes(p2p::NodeID const& _peerID, h256s const& _hashes, Asking _asking, SubprotocolPacketType _packetType)
{
    auto itPeerStatus = m_peers.find(_peerID);
    if (itPeerStatus == m_peers.end())
        return;
