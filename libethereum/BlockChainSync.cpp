...
{
    host().capabilityHost().foreachPeer(m_host.name(), m_host.version(), [this](NodeID const& _peerID)
    {
        syncPeer(_peerID, false);
        return true;
    });
...
void BlockChainSync::onPeerBlockHeaders(NodeID const& _peerID, RLP const& _r)
{
    ...
    if (m_daoChallengedPeers.find(_peerID) != m_daoChallengedPeers.end())
    {
        if (verifyDaoChallengeResponse(_r))
            ...
            m_host.capabilityHost().disableCapability(_peerID, CapDesc{m_host.name(), m_host.version()}, "Peer from another fork.");
...
void BlockChainSync::onPeerNewBlock(NodeID const& _peerID, RLP const& _r)
{
    ...
    if (_r.itemCount() != 2)
    {
        m_host.capabilityHost().disableCapability(_peerID, CapDesc{m_host.name(), m_host.version()}, "NewBlock without 2 data fields.");
        return;
    }
    ...
    case ImportResult::BadChain:
        logNewBlock(h);
        m_host.capabilityHost().disableCapability(_peerID, CapDesc{m_host.name(), m_host.version()}, "Malformed block received.");
        return;
    ...
    case ImportResult::UnknownParent:
    {
        ...
        {
            m_host.capabilityHost().disableCapability(_peerID, CapDesc{m_host.name(), m_host.version()}, "Too many uknown new blocks");
            restartSync();
        }
        logNewBlock(h);
