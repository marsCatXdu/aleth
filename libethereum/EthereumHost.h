...
public:
    ...
    EthereumHost(std::shared_ptr<p2p::CapabilityHostFace> _host, BlockChain const& _ch, OverlayDB const& _db,
...
//    void foreachPeer(std::function<bool(std::shared_ptr<EthereumPeer>)> const& _f) const;
    ...
    bool interpretCapabilityPacket(p2p::NodeID const& _peerID, unsigned _id, RLP const& _r) override;
    ...
    void requestStatus(p2p::NodeID const& _peerID, u256 _hostNetworkId, u256 _chainTotalDifficulty, h256 _chainCurrentHash, h256 _chainGenesPeersh);

    /// Request hashes for given parent hash.
    void requestBlockHeaders(p2p::NodeID const& _nodeID, h256 const& _startHash, unsigned _count, unsigned _skip, bool _reverse);
    void requestBlockHeaders(p2p::NodeID const& _nodeID, unsigned _startNumber, unsigned _count, unsigned _skip, bool _reverse);
...
private:
    ...
    std::tuple<std::vector<p2p::NodeID>, std::vector<p2p::NodeID>> randomSelection(unsigned _percent = 25, std::function<bool(EthereumPeerStatus const&)> const& _allow = [](EthereumPeerStatus const&){ return true; });
    ...
    void requestByHashes(p2p::NodeID const& _peerID, h256s const& _hashes, Asking _asking, SubprotocolPacketType _packetType);
