...
public:
    ...
    void onPeerNewHashes(p2p::NodeID const& _peerID, std::vector<std::pair<h256, u256>> const& _hashes);
...
private:
    ...
    std::set<p2p::NodeID> m_daoChallengedPeers; ///> Peers to which we've sent DAO challenge request
    ...
    std::map<p2p::NodeID, std::vector<unsigned>> m_headerSyncPeers; ///< Peers to m_downloadingSubchain number map
    std::map<p2p::NodeID, std::vector<unsigned>> m_bodySyncPeers; ///< Peers to m_downloadingSubchain number map
