...
public:
    ...
    void disableCapability(NodeID const& _nodeID, CapDesc const& _capDesc, std::string const& _problem) override
    {
        auto session = m_host.peerSession(_nodeID);
        if (session)
    ...
    RLPStream& prep(NodeID const& _nodeID, CapDesc const& _capDesc, RLPStream& _s, unsigned _id, unsigned _args = 0) override
    {
        auto session = m_host.peerSession(_nodeID);
        if (!session)
            return _s;
    ...
    void foreachPeer(std::string const& _name, u256 const& _version, std::function<bool(NodeID const&)> _f) const override
    {
